...
if(!empty($_POST)){
    ...
    if(empty($_POST['matricula']) || empty($_POST['Nombre']) || empty($_POST['Telefono']) || empty($_POST['Carrera'])){
...
$query = mysqli_query($mysqli,"SELECT * FROM alumno WHERE Matricula_A ='$matricula' OR Nombre_A = '$Nombre'");
 $result = mysqli_fetch_array($query);

  if($result > 0){
 $alert = '<p class="msg_error">El alumno ya existen.</p>';
   }else{
       $query_insert = mysqli_query($mysqli, "INSERT INTO alumno (Matricula_A, Nombre_A, Telefono_A, Carrera) VALUES('$matricula','$Nombre', '$Telefono', '$Carrera')");

     if($query_insert){
  $alert = '<p class="msg_error">El usuario a sido creado.</p>';
         }else{
